...
def friedman_test(*args):
    ...
    chi2 = ((12*n)/float((k*(k+1))))*((np.sum(np.fromiter((r**2 for r in rankings_avg), dtype=float)))-((k*(k+1)**2)/float(4)))
...
def friedman_aligned_ranks_test(*args):
    ...
    T = (k-1) * (np.sum(np.fromiter((v**2 for v in r_j), dtype=float)) - (k*n**2/4.) * (k*n+1)**2) / float(((k*n*(k*n+1)*(2*k*n+1))/6.) - (1./float(k))*np.sum(np.fromiter((v**2 for v in r_i), dtype=float)))
...
def quade_test(*args):
    ...
    Sj = [np.sum(np.fromiter((row[j] for row in S), dtype=float)) for j in range(k)]
    Wj = [np.sum(np.fromiter((row[j] for row in W), dtype=float)) for j in range(k)]
    
    rankings_avg = [w / (n*(n+1)/2.) for w in Wj]
    rankings_cmp = [r/np.sqrt(k*(k+1)*(2*n+1)*(k-1)/(18.*n*(n+1))) for r in rankings_avg]

    A = np.sum(np.fromiter((S[i][j]**2 for i in range(n) for j in range(k)), dtype=float))
    B = np.sum(np.fromiter((s**2 for s in Sj), dtype=float))/float(n)
    F = (n-1)*B/(A-B)
